...
def generate_reports():
    ...
    if not exec_status_path.exists():
        print(
            "specs/execution-status.yaml não encontrado. "
            "Rode o sync-status primeiro."
        )
        return
    ...
    for yaml_path in sorted(epics_dir.glob("e*.yaml")):
        ...
        for story in data.get("stories") or []:
            ...
            # Status da story no execution-status.yaml (default: planned)
            ...
            for task in story.get("tasks") or []:
                ...
                # Mapeamento do status para o Allure:
                # planned/pending/active aparecem como pendentes.
                if story_status in ["planned", "pending"]:
                    ET.SubElement(testcase, "skipped", {
                        "message": "Story is planned - Not yet implemented."
                    })
                elif story_status == "active":
                    ET.SubElement(testcase, "skipped", {
                        "message": "Story is active - In development."
                    })
    ...
    print(
        "Relatório de progresso BigPowers JUnit "
        f"gerado com sucesso em: {output_file}"
    )
