# e52s04: Auth Namespacing + Site→Project Backfill

**Story ID:** e52s04 | **Epic:** e52 — Project Scoping Backend | **BCPs:** 7 | **Status:** planned
...
This is the capstone story of e52. It wires together all previous stories:
- e52s01 provided kernel project ID context helpers
- e52s02 created the `projects` table with CRUD
- e52s03 added `project_id` columns to sites and deployments
...
- **Depends on:** e52s01 (kernel project ID helpers), e52s02 (projects table), e52s03 (project_id columns on sites/deployments)
- **Depended on by:** e56 (OTP session hardening needs project scope), e57 (UI), e58 (Neon port), e59 (Secrets), e60 (CSP), e61 (Dashboard), e62 (Schema Designer), e63 (Preview envs), e64 (Multi-user)
...
7. Manual smoke test:
   ...
   c. Verify token has project_id: decode JWT payload (base64url), confirm `"project_id"` exists
