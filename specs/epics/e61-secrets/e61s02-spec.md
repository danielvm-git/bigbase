# e61s02: Secrets UI in Admin Panel
## Story ID: e61s02 | Epic: e61 | BCPs: 1 | Status: planned

## 1. Type
**feat** · ui · domain

## 2. Context
With the backend `project_secrets` table and CRUD API implemented in `e61s01`, administrators and developers need a graphical interface to configure project-level secrets. This story implements the Secrets tab inside the `ProjectDetailPage` console view, reusing UI primitives and matching the UX of the existing `SiteEnvVarsTab`.
...
2. **Tab Integration**:
   Integrate `ProjectSecretsTab` into `ProjectDetailPage.tsx` (the project details view from `e58s01`) as a dedicated "Secrets" tab.
3. **Data Library**:
...
## 7. Dependencies
- **e58s01 (Project Scoping Admin UI)**: Expects the existence of `ProjectDetailPage.tsx` to insert the new tab.
- **e61s01 (Encrypted Secrets Storage & API)**: Requires the project secrets backend endpoints.
...
## 13. API / Data Model
Matches the types defined in `e61s01`:
```typescript
