# e57s01: Project Scoping Admin UI

## Story ID: e57s01 | Epic: e57 | BCPs: 3 | Status: planned
...
storage, messaging, and settings — but no page for managing **projects**. With e52
...
| Problem | Impact |
|---------|--------|
| No admin UI to manage projects | Users can't create or organize projects after e52 backend ships |
...
- `GET /api/auth/projects` — list projects (org-scoped)  
  _Note: If e52s02 is done, this endpoint already exists. If not, this file provides it._
...
- **e52s02 (Projects Table CRUD)** — backend `projects` CRUD endpoints. If e52s02 is not done,
  this story provides the API endpoints in `components/auth/projects_api.go`.
- **e52s03 (DB Isolation)** — `project_id` column on sites/deployments. The project detail
...
## 9. Risks
- **e52 not done yet**: The backend API endpoints may not exist. Mitigation: this story
  includes the full API endpoints in `components/auth/projects_api.go`. If e52s02 is
  already deployed, this file is not needed. Mark the file as `// e57s01: project CRUD API`.
...
1. Add `components/auth/projects_api.go` with project CRUD + sites/deployments listing endpoints
   (if e52s02 not yet deployed — otherwise skip, endpoints already exist).
...
**New endpoints (if e52s02 not yet deployed):**
```
...
| File | Change |
|------|--------|
| `components/auth/projects_api.go` | **NEW** (if e52s02 not done) — project CRUD + sites/deployments listing |
...
3. Delete `projects_api.go` if new (or revert route registration if it exists from e52).
4. Rebuild UI: `cd ui && npm run build`.
5. Deploy. All pages still render.

## 17. Acceptance Criteria

- [ ] Go backend exposes project CRUD endpoints (either from e52s02 or new in this story):
...
## 18. Implementation Steps (see e57s01-tasks.yaml)
